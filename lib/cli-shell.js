...
// types
function customCli({moduleName, defaultActionToExecute, addCliOptions, plainTextRegex, plainTextObject}) {
  this.moduleName = moduleName;
  this.defaultActionToExecute = defaultActionToExecute;
  this.plainTextRegex = plainTextRegex;
...
function readFromInputChannels (stdin, moduleName, plainTextRegex, plainTextObject) {
  ...
  if (/^[\],:{}\s]*$/.test(input.toString('utf8').replace(/\\["\\\/bfnrtu]/g, '@').
    ...
    if (typeof plainTextRegex === 'function' && typeof plainTextObject === 'function') {
      debug('applying the custom plainTextRegex function to each line of config for the module:' + moduleName);
      config = config.map(plainTextRegex).filter(i => !isempty(i)).map(plainTextObject);
    }
  }
...
function cmd (ci) {
  return new Promise((s, e) => {
    readStdin().then(stdin => {
      ...
      program.inputData = readFromInputChannels(stdin, ci.moduleName, ci.plainTextRegex, ci.plainTextObject);
